...
public class GetTotalDeathData {
    ...
    public static void getJsonfileDataToArray() throws IOException {
        for(int i = 24;i<Arrays.dates.size();i++){
            ...
            String filePath = "src\\main\\resources\\TotalDeathData\\jsonData\\date\\"+date+"TotalDeathData.json";
    ...
    */
    public static void storeAllCovid19Data(String city,String date,String confirm){
        ...
        for(int i = 1;i<Arrays.cities.size()+1;i++){
            if(Arrays.allCovid19Data_TotalDeath[i][0].equals(city))
                flag1 = i;
        }
        for(int j = 1;j<Arrays.dates.size()+1;j++){
            if(Arrays.allCovid19Data_TotalDeath[0][j].equals(date))
                flag2 = j;
        }
        Arrays.allCovid19Data_TotalDeath[flag1][flag2]=confirm;
    }
    public static void processJsonStr(String jsonStr,String city,String cityCodes) {
        ...
        if (jsonArray != null)
            for (int jsonCount = 0; jsonCount < jsonArray.size(); jsonCount++) {
                ...
                int integer = (Integer.parseInt(detail.getString("deathsNum"))  );
                if (integer < 0)
                    integer = 0;
                ...
                if (Arrays.isDateMatch(date, Arrays.dates)) {
                    for (int loop3 = 0; loop3 < Arrays.covid19MapDailies_TotalDeath.size(); loop3++) {
                        if (date.equals(Arrays.covid19MapDailies_TotalDeath.get(loop3).getDate())) {
                            Arrays.covid19MapDailies_TotalDeath.get(loop3).map.put(city, confirm);
                            break;
                        }
                    }
                }
    ...
    public static void processMapToJsonData() throws IOException {
        ...
        for(int i = 0;i<Arrays.covid19MapDailies_TotalDeath.size();i++) {
            String date = Arrays.covid19MapDailies_TotalDeath.get(i).getDate();

            //分时数据
            System.out.println(Arrays.covid19MapDailies_TotalDeath.get(i));
            System.out.println("json : ");
            JSONArray jsonArray = new JSONArray();
            List<String> keys = new ArrayList<>();
            List<String> values = new ArrayList<>();
            for(String key:Arrays.covid19MapDailies_TotalDeath.get(i).map.keySet())
            {
                keys.add(key);
                values.add(Arrays.covid19MapDailies_TotalDeath.get(i).map.get(key));
            }
            String answerJson = "[";
            String str1 = "{";
            String str2 = "}";
            ...
            for(int j = 0;j<Arrays.covid19MapDailies_TotalDeath.get(i).map.size();j++){
                if(j==Arrays.covid19MapDailies_TotalDeath.get(i).map.size()-1) {
    ...
    public static void writeAllCovid19DataTocsv() throws IOException {
        String filePath = "src\\main\\resources\\TotalDeathData\\csvData\\"+"TotalDeathData.csv";
        String content = "";
        System.out.println(filePath);
        for(int i = 0;i<Arrays.cities.size()+1;i++){
            content = Arrays.allCovid19Data_TotalDeath[i][0]+",";
            WriteToCsv.WriteTo(filePath,content);
            for(int j = 25;j<Arrays.dates.size()-1;j++){
                content = Arrays.allCovid19Data_TotalDeath[i][j]+",";
                WriteToCsv.WriteTo(filePath,content);

            }
            content = Arrays.allCovid19Data_TotalDeath[i][Arrays.dates.size()-1]+"\n";
            WriteToCsv.WriteTo(filePath,content);
        }
    }
}
